...
export default function generate(inputString) {
    const appSpec = JSON.parse(inputString);
    const className = appSpec.contract.name;
    const result = [];
    Object.keys(appSpec.hints).forEach((methodSignature) => {
        const methodName = methodSignature.split('(')[0];
        const callConfig = appSpec.hints[methodSignature].call_config;
        if (!['CALL', 'CREATE'].includes(callConfig.no_op))
            return;
        const { args } = appSpec.contract.methods.find((m) => m.name === methodName);
        ...
        const content = nunjucks.render(path.join(path.dirname(import.meta.url.replace('file:', '')), '..', 'templates', 'method.tsx.njk'), {
            className,
            methodName,
            methodSignature,
